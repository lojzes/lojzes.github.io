import{_ as t,X as i,Y as p,Z as n,a1 as s,$ as e,a2 as c,C as l}from"./framework-0b23a550.js";const o="/assets/circle-dependency-15f637e6.png",r="/assets/c-1-19c95a11.png",d="/assets/c-2-5e3f4cec.png",u="/assets/c-3-2173f466.png",v="/assets/c-chanjing-ad9d71a4.png",k="/assets/c-resolve-0ffd54f0.png",m="/assets/c-resolve-2-4f044cfb.png",b="/assets/c-resolve-3-6105ac3d.png",g="/assets/early-get-365b7e01.png",h="/assets/c-code-1-a1cf9985.png",y="/assets/c-resolve-4-e654d1e5.png",w="/assets/c-resolve-5-5a9f7a75.png",S="/assets/c-code-2-8a0c2a36.png",T="/assets/c-resolve-6-93557dfc.png",f="/assets/c-code-3-2cc947d2.png",j="/assets/c-4-aea75d90.png",_={},x={class:"hint-container tip"},A=n("p",{class:"hint-container-title"},"提示",-1),O={href:"https://mp.weixin.qq.com/s?__biz=MzkwNjMwMTgzMQ==&mid=2247490271&idx=1&sn=e4476b631c48882392bd4cd06d579ae9&source=41#wechat_redirect",target:"_blank",rel:"noopener noreferrer"},B={href:"https://www.modb.pro/db/215134",target:"_blank",rel:"noopener noreferrer"},z={href:"https://juejin.cn/post/6895753832815394824",target:"_blank",rel:"noopener noreferrer"},E=c('<h2 id="循环依赖问题在spring中主要有三种情况" tabindex="-1"><a class="header-anchor" href="#循环依赖问题在spring中主要有三种情况" aria-hidden="true">#</a> 循环依赖问题在Spring中主要有三种情况：</h2><ul><li>通过构造方法进行依赖注入时产生的循环依赖问题。</li><li>通过setter方法进行依赖注入且是在多例（原型）模式下产生的循环依赖问题。</li><li>通过setter方法进行依赖注入且是在单例模式下产生的循环依赖问题。</li></ul><p>注意：在Spring中，<u>只有【第三种方式】的循环依赖问题被解决了</u>，其他两种方式在遇到循环依赖问题时都会产生异常。</p><p>第一种构造方法注入的情况下，在new对象的时候就会堵塞住了，其实也就是”先有鸡还是先有蛋“的历史难题。</p><p>第二种setter方法&amp;&amp;多例的情况下，每一次<code>getBean()</code>时，都会产生一个新的<code>Bean</code>，如此反复下去就会有无穷无尽的Bean产生了， 最终就会导致<code>OOM</code>问题的出现。</p><h2 id="getbean-过程" tabindex="-1"><a class="header-anchor" href="#getbean-过程" aria-hidden="true">#</a> getBean 过程</h2><p><img src="'+o+`" alt="" loading="lazy"></p><h2 id="原因代码" tabindex="-1"><a class="header-anchor" href="#原因代码" aria-hidden="true">#</a> 原因代码</h2><p>先看看当时出问题的代码片段：</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token annotation punctuation">@Service</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TestService2</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这是一个经典的循环依赖，但是它能正常运行，得益于spring的内部机制，让我们根本无法感知它有问题，因为spring默默帮我们解决了。</p><p>spring内部有三级缓存：</p><ul><li>singletonObjects 一级缓存，用于保存实例化、注入、初始化完成的bean实例</li><li>earlySingletonObjects 二级缓存，用于保存实例化完成的bean实例</li><li>singletonFactories 三级缓存，用于保存bean创建工厂，以便于后面扩展有机会创建代理对象。</li></ul><p>下面用一张图告诉你，spring是如何解决循环依赖的：</p><p><img src="`+k+`" alt="" width="800" height="400" loading="lazy"></p><p>细心的朋友可能会发现在这种场景中第二级缓存作用不大。</p><p>那么问题来了，为什么要用第二级缓存呢？</p><p>试想一下，如果出现以下这种情况，我们要如何处理？</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code>
<span class="token annotation punctuation">@Service</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TestService1</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>为什么会循环依赖呢？</p><p>答案就在下面这张图中：</p><p><img src="`+w+'" alt="" loading="lazy"></p><p>说白了，bean初始化完成之后，后面还有一步去检查：第二级缓存 和 原始对象 是否相等。由于它对前面流程来说无关紧要，所以前面的流程图中省略了，但是在这里是关键点，我们重点说说：</p><p><img src="'+S+`" alt="" loading="lazy"></p><p>那位同事的问题正好是走到这段代码，发现第二级缓存 和 原始对象不相等，所以抛出了循环依赖的异常。</p><p>如果这时候把TestService1改个名字，改成：TestService6，其他的都不变。</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code>
<span class="token annotation punctuation">@Service</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>这个例子中本来如果TestService1和TestService2都没有加@DependsOn注解是没问题的，反而加了这个注解会出现循环依赖问题。</p><p>这又是为什么？</p><p>答案在AbstractBeanFactory类的doGetBean方法的这段代码中：</p><p><img src="`+f+'" alt="" loading="lazy"></p><p>它会检查dependsOn的实例有没有循环依赖，如果有循环依赖则抛异常。</p><h2 id="出现循环依赖如何解决" tabindex="-1"><a class="header-anchor" href="#出现循环依赖如何解决" aria-hidden="true">#</a> 出现循环依赖如何解决？</h2><p>项目中如果出现循环依赖问题，说明是spring默认无法解决的循环依赖，要看项目的打印日志，属于哪种循环依赖。目前包含下面几种情况：</p><p><img src="'+j+'" alt="" loading="lazy"></p><h3 id="生成代理对象产生的循环依赖" tabindex="-1"><a class="header-anchor" href="#生成代理对象产生的循环依赖" aria-hidden="true">#</a> 生成代理对象产生的循环依赖</h3><p>这类循环依赖问题解决方法很多，主要有：</p><ul><li>使用@Lazy注解，延迟加载</li><li>使用@DependsOn注解，指定加载先后关系</li><li>修改文件名称，改变循环依赖类的加载顺序</li></ul><h3 id="使用-dependson产生的循环依赖" tabindex="-1"><a class="header-anchor" href="#使用-dependson产生的循环依赖" aria-hidden="true">#</a> 使用@DependsOn产生的循环依赖</h3><p>这类循环依赖问题要找到@DependsOn注解循环依赖的地方，迫使它不循环依赖就可以解决问题。</p><h3 id="多例循环依赖" tabindex="-1"><a class="header-anchor" href="#多例循环依赖" aria-hidden="true">#</a> 多例循环依赖</h3><p>这类循环依赖问题可以通过把bean改成单例的解决。</p><h3 id="构造器循环依赖" tabindex="-1"><a class="header-anchor" href="#构造器循环依赖" aria-hidden="true">#</a> 构造器循环依赖</h3><p>这类循环依赖问题可以通过使用@Lazy注解解决。</p>',130);function C(P,I){const a=l("ExternalLinkIcon");return i(),p("div",null,[n("div",x,[A,n("p",null,[s("参考 "),n("a",O,[s('"spring：我是如何解决循环依赖的？"'),e(a)])]),n("p",null,[n("a",B,[s("说清楚Spring如何解决循环依赖？"),e(a)])]),n("p",null,[n("a",z,[s("如何解决循环依赖问题"),e(a)])])]),E])}const q=t(_,[["render",C],["__file","循环依赖.html.vue"]]);export{q as default};
